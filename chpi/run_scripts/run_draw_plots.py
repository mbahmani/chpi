...
dir = os.getcwd()
sys.path.append(dir)
from chpi.tools.helper import obtain_marginal_contributions, marginal_plots, determine_relevant
...
def draw_plots():
    ...
    for cls in {"optics"}:
        ...
        for metric in { "normalized_mutual_info_score", "silhouette_score", "davies_bouldin_score", "calinski_harabasz_score", "dunn_score", "c_index_score", "ratkowsky_lance_score", "mc_clain_rao_score"}:
            df = pd.read_csv("chpi/performance_data/"+cls+"_"+metric+"_fANOVA_results.csv")
